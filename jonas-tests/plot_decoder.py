...
for k, model in enumerate(model_label):
    ...
    plt.tight_layout()
    fig_full.savefig(
        f"{parent_directory}/results/jonas-cp-tests/decoder/compare_vanilla_{model}.pdf",
        bbox_inches="tight",
    )
    fig_split.savefig(
        f"{parent_directory}/results/jonas-cp-tests/decoder/compare_split_{model}.pdf",
        bbox_inches="tight",
    )
    fig_full.clf()
    fig_split.clf()
