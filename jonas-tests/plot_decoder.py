...
for k, model in enumerate(model_label):
    ...
    plt.tight_layout()
    fig_full.savefig(f"/g/g14/katona1/mphys-surrogate-model/jonas-tests/cp/decoder/compare_vanilla_{model}.pdf",
                   bbox_inches='tight') 
    fig_split.savefig(f"/g/g14/katona1/mphys-surrogate-model/jonas-tests/cp/decoder/compare_split_{model}.pdf",
                   bbox_inches='tight') 
    fig_full.clf()
    fig_split.clf()