...
for k, model in enumerate(model_label):
    ...
    for i, id in enumerate(test_ids_raw):
        ...
        for k_alpha, alpha in enumerate(alphas): # ensure masses are always non-negative
            ax_full_m[k][i].fill_between(dsd_time, np.maximum(0, m_lower_full[k, k_alpha, test_ids[i]]), 
                m_upper_full[k, k_alpha, test_ids[i]], color=colors[k_alpha], alpha=0.3, label=f"{100*(1-alpha)}% coverage")    
            ax_split_m[k][i].fill_between(dsd_time, np.maximum(0, m_lower_split[k, k_alpha, id]), 
                m_upper_split[k, k_alpha, id], color=colors[k_alpha], alpha=0.3, label=f"{100*(1-alpha)}% coverage")   
        ax_full_m[k][i].plot(dsd_time, m_rep_full[k, test_ids[i]], label="Model") # representative band
        ax_split_m[k][i].plot(dsd_time, m_rep_split[k, id], label="Model") # representative band
        for j in range(3):
            ...
            for k_alpha, alpha in enumerate(alphas):
                ax_full[j][i].fill_between(dsd_time, lower_full[k, k_alpha, test_ids[i], :, j], upper_full[k, k_alpha, test_ids[i], :, j], 
                    color=colors[k_alpha], alpha=0.3, label=f"{100*(1-alpha)}% coverage") 
                ax_split[j][i].fill_between(dsd_time, lower_split[k, k_alpha, id, :, j], upper_split[k, k_alpha, id, :, j], 
                    color=colors[k_alpha], alpha=0.3, label=f"{100*(1-alpha)}% coverage") 
            ax_full[j][i].plot(dsd_time, rep_full[k, test_ids[i], :, j], label=model) # representative band
            ax_split[j][i].plot(dsd_time, rep_split[k, id, :, j], label=model) # representative band
        ax_full[0][i].set_title(f"Sample #{test_ids[i]}")
        ax_full[-1][i].set_xlabel("t [s]")
        ax_split[0][i].set_title(f"Sample #{test_ids[i]}")
        ax_split[-1][i].set_xlabel("t [s]")
        ax_full_m[0][i].set_title(f"Sample #{test_ids[i]}")
        ax_full_m[-1][i].set_xlabel("mass [kg]")
        ax_split_m[0][i].set_title(f"Sample #{test_ids[i]}")
        ax_split_m[-1][i].set_xlabel("mass [kg]")
    ...
    plt.tight_layout()
    fig_full.savefig(f"/g/g14/katona1/mphys-surrogate-model/jonas-tests/cp/latent/compare_vanilla_{model}.pdf",
                   bbox_inches='tight') 
    fig_split.savefig(f"/g/g14/katona1/mphys-surrogate-model/jonas-tests/cp/latent/compare_split_{model}.pdf",
                   bbox_inches='tight') 
    fig_full.clf()
    fig_split.clf()
...
plt.tight_layout()
fig_full_m.savefig(f"/g/g14/katona1/mphys-surrogate-model/jonas-tests/cp/latent/compare_vanilla_m.pdf",
                   bbox_inches='tight') 
fig_split_m.savefig(f"/g/g14/katona1/mphys-surrogate-model/jonas-tests/cp/latent/compare_split_m.pdf",
                   bbox_inches='tight') 