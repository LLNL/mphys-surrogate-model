...
def compute_sim_error(X_train, X_test, vae, T, params, outdir, case_name):
    ...
    ztr_encoded = (
        ztr_encoded.reshape(X_train.shape[0], -1, params["latent_dim"]).detach().numpy()
    )
    ...
    for i in range(len(X_test)):
        ...
        x_0 = X_test[i, 0]
        x_sim = get_psd_evolution(
            x_0, coeff, vae, T, zlim, p_order=params["poly_order"]
        )
        for j, t in enumerate(T):
            w1_dist[i, j] = wasserstein_distance(X_test[i, j], x_sim[j])
            l2_err[i, j] = np.linalg.norm(X_test[i, j] - x_sim[j])

    np.savez(outdir + "/metrics/" + case_name + ".npz", l2_err=l2_err, w1_dist=w1_dist)
...
def retrain_sindy(ztr_encoded, T, output_dir, case_name, poly_order):
    ...
    try:
        ...
        with open(
            output_directory + "/sindy/" + case_name + ".pkl", "rb"
        ) as pickle_file:
...
def train_model(
    ...
    (data, norms, data_loaders) = du.create_e2e_dataloader(
        ds_all, cnn=params["CNN"], batch_size=params["batch_size"], tvt_split=split
    )
    ...
    (
        ...
        val_loss,
        val_losses,
    ) = training.train_network_e2e(
        train_data, params, val_dataloader=val_data, device=params["device"]
    )
    ...
    # vae model
    torch.save(
        vae.state_dict(), output_directory + "/autoencoder/" + case_name + ".pth"
    )
...
if eval_models:  # don't train, only evaluate
    ...
    for filename in glob.glob(output_directory + "/CNN_nl3_order2*.pkl"):
        ...
        case_name = (
            prefix
            + "_nl{}_order{}_tr{}-{}-{}_lr{}_weights{}-{}-{}-{}_{}".format(
                params["latent_dim"],
                params["poly_order"],
                params["pretraining_epochs"],
                params["training_epochs"],
        ...
        vae = models.CNNAutoEncoder(
            n_channels=1, n_bins=63, n_latent=params["latent_dim"]
        )
        device = torch.device(device)
        vae.load_state_dict(
            torch.load(
                output_directory + "/autoencoder/" + case_name + ".pth",
                map_location=device,
            )
        )

        split = (90, 10, 0)
        (data, _, _) = du.create_e2e_dataloader(
            ds_all, cnn=params["CNN"], batch_size=params["batch_size"], tvt_split=split
        )
        (X_train, DX, T) = data
        (data, _, _) = du.create_e2e_dataloader(
            ds_test, cnn=params["CNN"], batch_size=params["batch_size"], tvt_split=split
        )
        (X_test, DX, T) = data

        W1_err = compute_sim_error(
            X_train,
            X_test,
            vae,
            T,
            params,
            "../trained_models/hyperparam_e2e",
            case_name,
        )
