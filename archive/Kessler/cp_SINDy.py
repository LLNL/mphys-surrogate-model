...
if method[:5] == "split":
    ...
    if (p <= 0) | (p >= 100):
        raise ValueError(
            "Calibration size for split must be a percent strictly between 0 and 100."
        )
    method = "split"
if method[:3] == "cv+":
    k = int(method[3:])
...
def fun_damped(model, t, x):
    ...
    final_eps = (
        eps * np.min(abs_coefs[abs_coefs > 0]).flatten()
...
idx_train, idx_test = train_test_split(idx_all, test_size=test_size, random_state=1952)
...
if method == "jackknife":
    ...
    for i in range(n):
        ...
        idx = np.arange(n) != i
        m.fit(
            list(states_train[idx]),
            t=list(times_train[idx]),
            multiple_trajectories=True,
        )
        loo_preds[i] = evolve_states(
            model=m,
            initial_states=states_train[i, 0].reshape(1, -1),
            times=times_train[i].reshape(1, -1),
        )[0, 1:]
    ...
    preds = evolve_states(
        model=model_full, initial_states=states_test[:, 0], times=times_test
    )[:, 1:]
...
if method == "full":
    ...
    pred_train = evolve_states(
        model=m, initial_states=states_train[:, 0], times=times_train
    )[:, 1:]
    ...
    pred_test = evolve_states(
        model=m, initial_states=states_test[:, 0], times=times_test
    )[:, 1:]
...
if method == "split":
    ...
    pred_cal = evolve_states(model=m, initial_states=states_cal[:, 0], times=times_cal)[
        :, 1:
    ]
...
if method == "cv+":
    ...
    for train_idx, val_idx in kf.split(states_train):
        ...
        pred_val = evolve_states(
            model=m_k, initial_states=y0_val, times=times_train[val_idx]
        ...
        pred_test = evolve_states(
            model=m_k, initial_states=y0_test, times=times_test
        )  # (n_test, T_test, D)
