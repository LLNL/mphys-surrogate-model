...
parser = argparse.ArgumentParser(description="Run E-SINDy plotting, serially")

parser.add_argument("file_path", help="Path to your processed .npy file")
...
for i in range(len(times)):
    ...
    deriv = dfd._differentiate(
        states[i], times[i]
    )  # each is (n_timesteps, n_state_vars)
...
fit_model = model.fit(
    flat_states, x_dot=derivatives
)  # run on whole data first to get # of output features and preallocate
...
for b in range(B):
    indices = np.random.randint(
        0, len(flat_states), size=len(flat_states)
    )  # indices to select for bootstrap
    model.fit(
        flat_states[indices], x_dot=derivatives[indices]
    )  # solve model on resampled dataset
    for j in range(M):
        t_j = times[j]
        N = states[j].shape[1]
...
for j in range(M):
    ...
    for idx in np.ndindex(trajectory.shape):
        quantiles[j][idx] = np.array(
            [trajectory[idx].quantile(q) for q in quantile_query]
        )
