...
def process_trajectory(traj_no):
    ...
    axs[0].plot(t, sol_y[:, 0], "r-x", markersize=3, label=f"SINDy ({mode}) $q_c$")
    ...
    axs[1].plot(t, sol_y[:, 1], "r-x", markersize=3, label=f"SINDy ({mode}) $q_r$")
...
def main():
    ...
    with mp.Pool(
        processes=mp.cpu_count(), initializer=init_worker, initargs=init_args
    ) as pool:
