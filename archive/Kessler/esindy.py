...
fit_model = model.fit(
    states, x_dot=derivatives
)  # run on whole data first to get # of output features and preallocate
bootstraps = np.zeros(
    (q, N, model.n_output_features_)
)  # preallocate coefficient samples for each bootstrap
for i in range(q):  # sample with replacement q times
    indices = np.random.randint(
        0, len(states), size=len(states)
    )  # indices to select for bootstrap
    model.fit(
        states[indices], x_dot=derivatives[indices]
