...
with open(file_path.removesuffix("processed.npy")+'sindy_model.pkl', 'rb') as f:
    model = dill.load(f)
    ...
    try:
        ...
        for i, ti in enumerate(t):
            if i == 0: # save initial condition first
                ...
                sol = solve_ivp(fun, (t[i-1], ti), sol_y[i-1], method='LSODA', t_eval=[ti])
                # print('Solver status:', sol.status) # print termination condition
                if sol.status == -1:
                    raise Exception("Integration failed, trying damped version.") # raise a custom exception for LSODA
        ...
        # First subplot: q_c
        axs[0].plot(t, states_j[:, 0], color='blue', marker='o', markersize=3, label=r'Exact, $q_c$')
        axs[0].plot(t, sol_y[:, 0], color='orange', marker='x', markersize=3, label=r'SINDy (non-damped), $q_c$')
        ...
        # Second subplot: q_r
        axs[1].plot(t, states_j[:, 1], color='blue', marker='o', markersize=3, label=r'Exact, $q_r$')
        axs[1].plot(t, sol_y[:, 1], color='orange', marker='x', markersize=3, label=r'SINDy (non-damped), $q_r$')
        ...
        plt.tight_layout()
        plt.savefig(file_path.removesuffix("_processed.npy")+"/SINDy/trajectory"+str(traj_no)+".pdf")
        ...
        try:
            for i, ti in enumerate(t):
                if i == 0: # save initial condition first
                    ...
                    # integrate from t[i-1] to t[i]
                    sol = solve_ivp(fun_damped, (t[i-1], ti), sol_y[i-1], method='LSODA', t_eval=[ti])
                    # print('Solver status:', sol.status) # print termination condition
                    if sol.status == -1:
                        raise Exception("Damped integration failed.") # raise a custom exception for LSODA
                    sol_y[i] = sol.y.flatten()
                    # make solution non-negative
                    sol_y[i] = np.maximum(sol_y[i], 0)
            ...
            # First subplot: q_c
            axs[0].plot(t, states_j[:, 0], color='blue', marker='o', markersize=3, label=r'Exact, $q_c$')
            axs[0].plot(t, sol_y[:, 0], color='orange', marker='x', markersize=3, label=r'SINDy (damped), $q_c$')
            axs[0].set_xlabel("t [seconds]")
            axs[0].set_ylabel("$q_{c}$ [kg liquid/kg air]")
            axs[0].legend(bbox_to_anchor=(1.05, 1), loc='upper left')
            axs[0].grid(True)
            ...
            # Second subplot: q_r
            axs[1].plot(t, states_j[:, 1], color='blue', marker='o', markersize=3, label=r'Exact, $q_r$')
            axs[1].plot(t, sol_y[:, 1], color='orange', marker='x', markersize=3, label=r'SINDy (damped), $q_r$')
            axs[1].set_xlabel("t [seconds]")
            axs[1].set_ylabel("$q_{r}$ [kg liquid/kg air]")
            axs[1].legend(bbox_to_anchor=(1.05, 1), loc='upper left')
            axs[1].grid(True)
