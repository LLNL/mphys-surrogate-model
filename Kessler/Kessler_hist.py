...
for i in range(bootstraps.shape[0]): # loop through each variable i
    for j in range(bootstraps.shape[1]): # coefficient j for ODE for variable i
        ...
        plt.title("Variable "+str(i)+", feature "+str(j)+": "+str(features[j]), fontsize='x-large')
        ...
        output_filename = file_path.removesuffix("_esindy.pkl")+"/E-SINDy/histogram"+str(i)+"feature"+str(j)+".pdf"
