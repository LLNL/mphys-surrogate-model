...
def main():
    ...
    p.add_argument("-j", "--n_jobs", type=int,
    ...
    ders = [ps.FiniteDifference()._differentiate(s, t)
    ...
    for j in range(M):
        ...
        # First subplot: q_c
        axs[0].plot(t, states[traj_nos[j]][:, 0], color='blue', marker='o', markersize=0.5, label=r'Exact, $q_c$')
        ...
        # Second subplot: q_r
        axs[1].plot(t, states[traj_nos[j]][:, 1], color='blue', marker='o', markersize=0.5, label=r'Exact, $q_r$')
        ...
        plt.tight_layout()
        plt.savefig(args.file_path.removesuffix("_processed.npy")+
                    "/E-SINDy/CI/intervals"+str(traj_nos[j])+".pdf")
        plt.close()
