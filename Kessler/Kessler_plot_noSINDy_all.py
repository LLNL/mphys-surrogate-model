...
for traj_no in range(len(times)):
    ...
    try:
        # First subplot: q_c
        axs[0].plot(t, states[traj_no][:, 0], color='blue', marker='o', markersize=3, label=r'Exact, $q_c$')
        ...
        # Second subplot: q_r
        axs[1].plot(t, states[traj_no][:, 1], color='blue', marker='o', markersize=3, label=r'Exact, $q_r$')
        ...
        plt.tight_layout()
        plt.savefig(file_path.removesuffix("_processed.npy")+"/noSINDy/trajectory"+str(traj_no)+".pdf")
