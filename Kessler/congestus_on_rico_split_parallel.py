...
def main():
    ...
    parser.add_argument("-s", "--cal_size", required=True,
    ...
    pred_cal = evolve_states_parallel(model=m, initial_states=states_cal[:,0], times=times_cal, n_workers=total_cpus)[:,1:]   
    ...
    pred_test = evolve_states_parallel(model=m, initial_states=states_test[:,0],  times=times_test, n_workers=total_cpus)[:,1:]   
