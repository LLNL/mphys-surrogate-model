...
def multivar_ccf_fft_squared(X, Y, maxlags=None,
                             unbiased=False, demean=True,
                             standardize=True):
    X = np.asarray(X)
    Y = np.asarray(Y)
    T, p = X.shape
    ...
    # 2) standardize
    if standardize:
        sx = x.std(axis=0, ddof=0); sx[sx==0] = 1.0
        sy = y.std(axis=0, ddof=0); sy[sy==0] = 1.0
    else:
        sx = np.ones(p); sy = np.ones(q)
    # 3) FFT length
    full_lags = T - 1
    ...
    # 6) re‐index lags
    idx = np.r_[np.arange(nfft-full_lags, nfft),
...
def cross_lb_fft_test(X, Y, m=None,
                      ccf_fft=multivar_ccf_fft_squared,
                      tol=None, ar_order=None):
    """
...
def _compute_residual(i):
    ...
    for j in range(nv):
        out[:, j] = AutoReg(ts[:, j+1],
                            lags=_ar_order,
                            old_names=False).fit().resid
...
def main():
    ...
    neighbors = [[j for j in nbrs if j!=i]
                 for i,nbrs in enumerate(ball)]
