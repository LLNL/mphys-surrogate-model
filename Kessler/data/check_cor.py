...
neighbor_list = [
    ...
    for i,nbrs in enumerate(ball_lists)
...
dset_residuals = np.empty((dset.shape[0], dset.shape[1] - p, dset.shape[2]), dtype=float) # get residuals for model
...
for i, ts in enumerate(dset):
    tmp = np.array([prewhiten(ts[:,j+1], p) for j in range(len(ts[0])-1)]).T # save residuals
    dset_residuals[i] = np.hstack((ts[p:, 0:1], tmp)) # concatenate residuals with times
...
def multivar_ccf_fft_squared(X, Y, maxlags=None, unbiased=False, demean=True, standardize=True):
    """
    ...
    idx = np.concatenate((np.arange(nfft-full_lags, nfft),
                          np.arange(       0, full_lags+1)))
...
def cross_lb_fft_test(X, Y, m=None, ccf_fft=multivar_ccf_fft_squared, tol=None, ar_order=ar_order):
    """
...
linear_pvals = neighbor_list # preallocate p-values for linear fit between neighbors and gridbox
for i, ts_i in enumerate(dset_residuals[:, :, 1:]): # for each gridbox i and time series ts_i...
    ts_neighbors = dset_residuals[neighbor_list[i], :, 1:] # get all time series for neighbors
    # compute CCF values
    for j, ts_j in enumerate(ts_neighbors):
        ...
        CCFs_pvals[i][j] = list((cross_lb_fft_test(ts_i, ts_j, m=int(round(len(ts_i) ** (1/3))), tol=tol)[k] for k in (0,1,-1)))
