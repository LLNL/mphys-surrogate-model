...
path = '/g/g14/katona1/mphys-surrogate-model/jonas-tests-boxcox/cp/decoder'
    ...
    for i, id in enumerate(test_ids_raw):
        for j, t in enumerate(tplt):
            ...
            for k_alpha, alpha in enumerate(alphas):
                if k == 0:
                    ax_full[j][i].fill_between(r_bins_edges, np.maximum(0, lower_full[k, k_alpha, test_ids[i], t]), upper_full[k, k_alpha, test_ids[i], t], 
                        step='pre', color=colors[k_alpha], alpha=0.3, label=f"{100*(1-alpha)}% coverage") 
                    ax_split[j][i].fill_between(r_bins_edges, np.maximum(0, lower_split[k, k_alpha, id, t]), upper_split[k, k_alpha, id, t], 
                        step='pre', color=colors[k_alpha], alpha=0.3, label=f"{100*(1-alpha)}% coverage") 
                else:
                    ax_full[j][i].fill_between(r_bins_edges, np.maximum(0, lower_full[k, k_alpha, test_ids[i], t]), upper_full[k, k_alpha, test_ids[i], t], 
                        step='pre', color=colors[k_alpha], alpha=0.3)
                    ax_split[j][i].fill_between(r_bins_edges, np.maximum(0, lower_split[k, k_alpha, id, t]), upper_split[k, k_alpha, id, t], 
                        step='pre', color=colors[k_alpha], alpha=0.3)
            ax_full[j][i].step(r_bins_edges, rep_full[k, test_ids[i], t], label=model) # representative band
            ax_split[j][i].step(r_bins_edges, rep_split[k, id, t], label=model) # representative band
            ax_full[j][i].set_xscale("log")
            ax_split[j][i].set_xscale("log")
        ax_full[0][i].set_title(f"Sample #{test_ids[i]}")
        ax_full[-1][i].set_xlabel("radius (um)")
        ax_split[0][i].set_title(f"Sample #{test_ids[i]}")
        ax_split[-1][i].set_xlabel("radius (um)")
for j, t in enumerate(tplt):
...
fig_full.suptitle("Box-Cox vanilla conformal predictions", fontsize=14)
fig_split.suptitle("Box-Cox split conformal predictions", fontsize=14)
plt.tight_layout()
fig_full.savefig("/g/g14/katona1/mphys-surrogate-model/jonas-tests-boxcox/cp/decoder/compare_vanilla.pdf")
fig_split.savefig("/g/g14/katona1/mphys-surrogate-model/jonas-tests-boxcox/cp/decoder/compare_split.pdf")