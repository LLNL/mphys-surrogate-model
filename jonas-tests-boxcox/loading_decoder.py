...
def g_inv(y):  # inverse
    return np.where(λ!=0, (λ*y + 1)**(1/λ), np.exp(y))
...
DSD_train_all = g(encode_decode_ae(x_train))
print('Predicting the testing data.')
DSD_test = g(encode_decode_ae(x_test))
print('Running vanilla conformal predictions.')
for i in range(len(models)):
    DSD_res_signed = DSD_train_all[i] - g(x_train)
    DSD_q_low, DSD_q_high = one_sided_quantiles(DSD_res_signed, alpha_lows, alpha_ups) 
    DSD_lower_full[i] = g_inv(DSD_test[i, np.newaxis, ...] - DSD_q_high[:, np.newaxis, ...])
    DSD_upper_full[i] = g_inv(DSD_test[i, np.newaxis, ...] - DSD_q_low [:, np.newaxis, ...])

# save lower, upper, and representative for DSD
DSD_bands_full = [DSD_lower_full, g_inv(DSD_test), DSD_upper_full]
...
DSD_cal = g(encode_decode_ae(x_cal))
print('Predicting the testing data.')
DSD_testing = g(encode_decode_ae(x_testing))
print('Running split conformal predictions.')
for i in range(len(models)):
    DSD_res_signed = DSD_cal[i] - g(x_cal)
    DSD_q_low, DSD_q_high = one_sided_quantiles(DSD_res_signed, alpha_lows, alpha_ups) 
    DSD_lower_split[i] = g_inv(DSD_testing[i, np.newaxis, ...] - DSD_q_high[:, np.newaxis, ...])
    DSD_upper_split[i] = g_inv(DSD_testing[i, np.newaxis, ...] - DSD_q_low [:, np.newaxis, ...])

# save lower, upper, and representative for DSD
DSD_bands_split = [DSD_lower_split, g_inv(DSD_testing), DSD_upper_split]
