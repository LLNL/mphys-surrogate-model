...
for p in "${P_VALUES[@]}"; do
    ...
    python3 UQ/conformal/ae_NNdzdt.py erf_data/RICO/noadv_coal_200m \
      -p "$p" -a 0.1 0.05 0.025 0.01 -j "$NJOBS_PER_P" \
      > "logs/RICO_p${p}_NNdzdt.log" 2>&1

    echo "congestus9600 split: ${p}-$((100 - p))"
    python3 UQ/conformal/ae_NNdzdt.py erf_data/congestus/noadv_coal_200m_9600 \
      -p "$p" -a 0.1 0.05 0.025 0.01 -j "$NJOBS_PER_P" \
      > "logs/congestus9600_p${p}_NNdzdt.log" 2>&1
    ...
    python3 UQ/conformal/ae_NNdzdt.py congestus_coal_200m_test \
      -p "$p_small" -a 0.1 0.05 0.025 0.01 -j "$NJOBS_PER_P" \
      > "logs/congestus_p${p_small}_NNdzdt.log" 2>&1
