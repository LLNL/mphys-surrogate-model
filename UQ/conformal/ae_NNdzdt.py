...
def run_ae_X_latent_batched(z_enc, m, model, zlim, n_jobs):
...
def run_ae_X_batched(z_enc, m, model, zlim, n_jobs, device):
    latents_all = run_ae_X_latent_batched(z_enc, m, model, zlim, n_jobs)  # (N, T, L+1)
...
def run_all_batched(outputs, model, z_train_enc, z_test_enc, zlim, device, n_jobs):
    ...
    DSD_train_full, M_train_full, Z_train_full = run_ae_X_batched(
        z_train_enc, outputs["m_train"], model, zlim, n_jobs, device
    )
