...
def train_network_e2e_bb(train_dataloader, params, val_dataloader=None, device="cpu", X=None, T=None):
    device = torch.device(device)

    if params["CNN"]:
        autoencoder_network = models.CNNAutoEncoder(n_channels=1, n_bins=params["input_dim"],
    ...
    optimizer = torch.optim.Adam([*deriv_weights, *deriv_biases,
                                  *encoder_weights, *encoder_biases,
                                  *decoder_weights, *decoder_biases
                                  ],
                                 lr=params["learning_rate"]
    ...
    ref_params['loss_weight_sindy_z'] = 0.0
    for epoch in range(params["pretraining_epochs"]):
        (epoch_loss, epoch_losses) = train_e2e_bb(optimizer,
                                               train_dataloader,
                                               ref_params,
                                               encoder_weights,
                                               encoder_biases,
    ...
    return autoencoder_network, derivative_network, train_loss, train_losses, val_loss, val_losses
...
def train_e2e_bb(optimizer, train_dataloader, params,
              encoder_weights, encoder_biases,
              decoder_weights, decoder_biases,
              deriv_weights, deriv_biases,
              autoencoder_network,
              derivative_network,
              device,
              cnn=False
...
def test_e2e_bb(val_dataloader, params,
              encoder_weights, encoder_biases,
              decoder_weights, decoder_biases,
              deriv_weights, deriv_biases,
              autoencoder_network,
...
def loss_fn_e2e_bb(x_data,
    ...
    if (params["loss_weight_sindy_z"] + params["loss_weight_sindy_x"]) > 0.0:
        ...
        z = autoencoder_network.encoder(xx)
        if cnn:
            gradient_x = torch.func.vmap(torch.func.jacrev(autoencoder_network.encoder, chunk_size=20), chunk_size=20)(
                xx)[:, 0, :, :, 0, :]
        else:
            gradient_x = torch.func.vmap(
                torch.func.vmap(torch.func.jacrev(autoencoder_network.encoder, chunk_size=20), chunk_size=20),
                chunk_size=20)(xx)
        ...
        if params["loss_weight_sindy_x"] > 0.0:
            ...
            if cnn:
                gradient_z = torch.func.vmap(
                    torch.func.vmap(torch.func.jacrev(autoencoder_network.decoder, chunk_size=20), chunk_size=20),
                    chunk_size=20)(z)[:, 0, 0, :, :, :]
            else:
                gradient_z = torch.func.vmap(torch.func.vmap(torch.func.jacrev(autoencoder_network.decoder)))(z)
